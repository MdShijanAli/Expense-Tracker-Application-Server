...
function fundsController() {
  ...
  const getUserSingleFundCategoryDataLists = async (req, res) => {
    ...
    try {
      const pageNum = parseInt(page);
      const limitNum = parseInt(limit);
      const result = await fundsModel.getUserSingleFundCategoryDataLists(category, userEmail, pageNum, limitNum, sort_by, sort_order, search);
      const total = result?.total?.length;
      if (result?.funds?.length > 0) {
        formatResultData({
          res,
  ...
  return {
    ...
    getFundsByDate,
    getUserSingleFundCategoryDataLists,
